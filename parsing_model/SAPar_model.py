...
class SAChartParser(nn.Module):
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False, k=1):
        '''
            ...
            for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
                ...
                label_scores_chart, span_attentions \
                    = self.label_scores_from_annotations(sub_fencepost_annotations_start,
                                                         ...
                                                         sentence)
                trees_scores = self.parse_from_annotations(label_scores_chart, sentence, golds[i], k=k)
                trees_list = [t for t, s in trees_scores]
                scores_list = [s for t, s in trees_scores]
                batch_trees.append(trees_list)
    ...
    def parse_from_annotations(self, label_scores_chart, sentence, gold=None, k=1):
        ...
        if is_train:
            ...
            return self.decode_from_chart(sentence, label_scores_chart_np, k=k)

    def decode_from_chart_batch(self, sentences, charts_np, golds=None, k=1):
        '''
        Decode a batch of charts.
        batch_trees: type: list of lists. Structure: each element is a list containing the parsing trees of the corresponding sentence. content: for each sentence, a sub-list of batch_trees contains k parsing trees for that sentence.
        batch_scores: type: list of lists. Structure: each element is a list containing the scores of the corresponding sentence. Contents: for each sentence, the sublists in batch_scores contain the k parse scores for that sentence.
        '''
        batch_trees = []
        batch_scores = []
        if golds is None:
        ...
        for sentence, chart_np, gold in zip(sentences, charts_np, golds):
            # trees: a list of parse trees and their corresponding scores. The format is usually [(tree1, score1), (tree2, score2), ... , (tree_k, score_k)].
            # Called decode_from_chart
            trees = self.decode_from_chart(sentence, chart_np, gold, k=k)
            batch_trees.append([t for t, s in trees])  # extract trees
            batch_scores.append([s for t, s in trees])  # extract scores
        return batch_trees, batch_scores  # return list of trees and list of scores

    def decode_from_chart(self, sentence, chart_np, gold=None, k=1):
        '''
        Decode k-best chart
        '''
        decoder_args = dict(
        ...
        scores, p_i_list, p_j_list, p_label_list, p_split_list, _ = chart_helper.decode(force_gold, **decoder_args)
        trees_scores = []

        for parse_idx in range(len(scores)):
            p_i = p_i_list[parse_idx]
            p_j = p_j_list[parse_idx]
            p_label = p_label_list[parse_idx]
            p_split = p_split_list[parse_idx]
            score = scores[parse_idx]

            span_to_index = {}
            for idx_span in range(len(p_i)):
                i_span, j_span = p_i[idx_span], p_j[idx_span]
                span_to_index[(i_span, j_span)] = idx_span

            def make_tree(i, j):
                idx = span_to_index.get((i, j))
                if idx is None:
                    return None

                label_idx = p_label[idx]
                label_value = self.label_vocab.value(label_idx)
                ...
                # If it's the root node, set the label to 'S'
                if i == 0 and j == len(sentence):
                    label = 'S'
                ...
                if (i + 1) == j:
                    ...
                    # Internal node
                    split = p_split[idx]
                    left_tree = make_tree(i, split)
                    right_tree = make_tree(split, j)
                    children = [left_tree, right_tree]

                    if label is not None:
                        tree = trees.InternalParseNode(label, children)
                        return tree
                    else:
                        # Label is None, use placeholder label 'NONE'
                        tree = trees.InternalParseNode(('NONE',), children)
                        return tree

            def prune_none_nodes(node):
                if isinstance(node, trees.LeafParseNode):
                    return node
                elif isinstance(node, trees.InternalParseNode):
                    # Recursively prune child nodes
                    pruned_children = []
                    for child in node.children:
                        pruned_child = prune_none_nodes(child)
                        if isinstance(pruned_child, list):
                            pruned_children.extend(pruned_child)
                        elif pruned_child is not None:
                            pruned_children.append(pruned_child)
                    # If current node's label is 'NONE', return its pruned children directly
                    if node.label == ('NONE',):
                        if len(pruned_children) == 0:
                            return None
                        elif len(pruned_children) == 1:
                            return pruned_children[0]
                        else:
                            return pruned_children  # Return list of children
                    else:
            ...
            # Build the tree
            tree = make_tree(0, len(sentence))
            # Prune 'NONE' nodes
            tree = prune_none_nodes(tree)

            # After pruning, handle the possibility that tree is a list
            if isinstance(tree, list):
                if len(tree) == 1:
                    tree = tree[0]
                else:
                    # Wrap in a root node
                    tree = trees.InternalParseNode(('S',), tree)
            elif tree is None:
                tree = trees.InternalParseNode(('S',), [])

            # Ensure the root node has label 'S'
            if tree.label != ('S',):
                tree = trees.InternalParseNode(('S',), [tree])

            trees_scores.append((tree, score))
        return trees_scores